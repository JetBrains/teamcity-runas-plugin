...
{	
	...
	help << std::endl << L"\t-b:" << ARG_EXIT_CODE_BASE << "\t- base number for exit code, it is optional and " << DEFAULT_EXIT_CODE_BASE << L" by default.";
	help << std::endl << L"\t-l:" << ARG_LOG_LEVEL << "\t\t- logging level (" << LOG_LEVEL_NORMAL << L"|" << LOG_LEVEL_ERRORS << L"|" << LOG_LEVEL_OFF << L"), it is optional and \"" << LOG_LEVEL_NORMAL << L"\" by default.";
	...
	help << std::endl << L"\t" << ARG_EXIT_COMMAND_LINE_ARGS << "\t- command line arguments, it is optional and empty by default. The maximum total length of \"" << ARG_EXECUTABLE << L"\" and \"" << ARG_EXIT_COMMAND_LINE_ARGS << L"\" is 1024 characters.";
	help << std::endl;
